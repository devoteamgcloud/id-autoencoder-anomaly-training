...
def preprocess(
    ...
    ohe_dfs = []
    for col, top_ntop_n in ohe_columns:
        
        if len(ohe_class_names[col]) > top_n:
            # Convert everything else outside top-n to "default" class  (in this case, nanin this case, nan)
...
def create_ohe_class_names(ohe_columns: List[Tuple[str, int]]) -> Tuple[Dict[str, List[str]], Dict[str, str]]:
    ...
    # Handle case wHandle case when number of classes exceeds top_n, create a new "n number of classes exceeds top_n, create a new "other" class" class
    ohe_classes = {}
